...
select t.train_id,t.train_name,t.source_stn,t.destination_stn,tr.available_date from Train t Left outer join Train_status tr on t.train_id=tr.train_id;


select t.train_id,t.train_name,t.source_stn,t.destination_stn from Train t
where t.train_id in( select tc.train_id from Train_status tc where tc.reserved_seats >=0);

select tc.train_id,tc.train_name,tc.source_stn,tc.destination_stn from Train tc
union all
select tr.train_id,tr.available_date from Train_status tr where tr.train_id in( select t.train_id from Train t where t.train_id>0);
