...
update Bus_status set reserved_seats=rv where bus_id=b_id;
update Reservation_status set reserved=rs where bus_id=b_id and seat_id=s_id;
else
...
insert into Waiting_list values(sl,cname,b_id);
pass_tck(sl,'y',b_id);
end if;
end;
/
...
select MIN(slno) into sno from waiting_list where bus_id=b_id;

if sno is not null then
select customer_name into cname from waiting_list where bus_id=b_id and slno=sno;
update reservation_status set customer_name=cname where bus_id=b_id and seat_id=sid;
delete from waiting_list where bus_id=b_id and slno=sno;
